...
- [x] Split the hook into its `&&` steps and compare **as a set of steps**, not
      as one string.
- [x] If every canonical step is present, in an order that still works
      (generators before `git add`, guard last), leave the script alone and
      report `skipped`.
- [x] If the guard step is missing, append it as the last step. It must be last:
...
- [x] If a canonical step is missing from the middle (say a consumer removed the
...
- [x] Keep `--force` as the "just write the canonical string" escape hatch.
- [x] Apply the same care to the helper scripts (`changelog`, `releases`,
...
- [x] A hook with an extra step (`prettier --write …` between generate and
      `git add`) keeps that step and gains the missing guard, in the right
      position.
- [x] A hook already carrying every canonical step is left byte-identical and
      reported as skipped.
- [x] A hook missing a generator step is warned about, not rewritten.
- [x] A project with no `version` script gets the canonical one, as today.
- [x] `--force` still replaces whatever is there.
- [x] **Stays-silent:** a project with release generation disabled in its config
